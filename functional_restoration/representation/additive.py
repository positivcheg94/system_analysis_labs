...
def __make_psi_polynom2__(lambdas, polynom, dims_x_i, p):
    def __make_basis__(degree):
        max_basis = []
        for deg in range(1, degree + 1):
            b = np.zeros(deg)
...
def __unshifted_f_polynoms__(f_real_polynoms, x_scales, y_scales):
...
def representation(polynom_type, p, dims_x_i, x_scales, y_scales, lambdas, a_small, c):
    ...
    unshifted_f = __unshifted_f_polynoms__(f_real, x_scales, y_scales)
